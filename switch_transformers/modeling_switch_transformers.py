...
class SwitchTransformersDenseActDenseForSparse(nn.Module):
    def __init__(self, config: SwitchTransformersConfig, krnl, context, queue):
        super().__init__()
    ...
    def upload_weights(self):
        #--- wi ------
        wi_weight = self.wi.weight.detach().numpy().T
        self.wi_pad = np.hstack((wi_weight, np.zeros((wi_weight.shape[0], (64-wi_weight.shape[1]%64)%64)))).astype(np.float16)
        self.wi_list = list()
        for i in range(0, wi_weight.shape[1], 64):
            self.wi_list.append(matmul_lib.moveMatB(self.queue, self.context, np.ascontiguousarray(self.wi_pad[:, i:i+64])))
        
        # #--- wo -------
        wo_weight = self.wo.weight.detach().numpy().T
        self.wo_pad = np.hstack((wo_weight, np.zeros((wo_weight.shape[0], (64-wo_weight.shape[1]%64)%64)))).astype(np.float16)
        self.wo_list = list()
        for i in range(0, wo_weight.shape[1], 64):
            self.wo_list.append(matmul_lib.moveMatB(self.queue, self.context, np.ascontiguousarray(self.wo_pad[:, i:i+64])))
        self.uploaded = True
    ...
    def forward(self, hidden_states):
        if(self.uploaded == False):
            ...
            hidden_states_t = np.matmul(hidden_states, np.array(self.wi.weight).T)
            hidden_states_t = self.act(torch.from_numpy(hidden_states_t))
            hidden_states = torch.from_numpy(np.matmul(np.array(hidden_states_t), np.array(self.wo.weight).T))
        else:
            #--- Handle with FPGA------------------------
            ...
            if(hidden_states.ndim == 3):
                hidden_states = hidden_states[0]
            hidden_states_pad = np.vstack((hidden_states, np.zeros(((4-hidden_states.shape[0]%4)%4, hidden_states.shape[1])))).astype(np.float16)
            
            answer = np.empty((hidden_states_pad.shape[0], self.wo_pad.shape[1]), dtype=np.float16)
            mat_ref = np.ndarray((0,0), dtype=np.float16)
            
            for i in range(0, hidden_states.shape[0], 4):
                input = matmul_lib.moveMatA(self.queue, self.context, np.ascontiguousarray(hidden_states_pad[i:i+4, :]))

                wi_target = matmul_lib.setMatResultHostArr()
                wi_buffer = matmul_lib.setMatResultBuffer(self.context, wi_target, len(self.wi_list))
                wo_target = matmul_lib.setMatResultHostArr()
                wo_buffer = matmul_lib.setMatResultBuffer(self.context, wo_target, len(self.wo_list))

                for j in range(0, len(self.wi_list)):
                    matmul_lib.setArgumentsAndEnqueue(self.krnl, self.queue, hidden_states.shape[1], input, self.wi_list[j], wi_buffer, 64*4*j, 0, 1)
                
                
                for j in range(0, len(self.wo_list)):
                    matmul_lib.setArgumentsAndEnqueue(self.krnl, self.queue, self.wi_pad.shape[1], wi_buffer, self.wo_list[j], wo_buffer, 64*4*j, bool(j == len(self.wo_list)-1), 0)
                
                
                answer[i:i+4, :] = np.ascontiguousarray(matmul_lib.getResultMatrix(self.queue, wo_buffer, wo_target, mat_ref, self.wo_pad.shape[1]))
                
                matmul_lib.freeBuffer(input)
                matmul_lib.freeBuffer(wi_buffer)
                matmul_lib.freeBuffer(wo_buffer)
            
            hidden_states = torch.from_numpy(answer[:hidden_states.shape[0], :self.wo.weight.detach().numpy().T.shape[1]])
        return hidden_states
...
class SwitchTransformersSparseMLP(nn.Module):
    ...
    def __init__(self, krnl, context, queue, config: SwitchTransformersConfig, expert_class: nn.Module = SwitchTransformersDenseActDenseForSparse):
        super().__init__()
        ...
        for idx in range(config.num_experts):
            self.experts[f"expert_{idx}"] = expert_class(config, krnl=krnl, context=context, queue=queue)
    
    def upload_weights(self):
        for expert in self.experts.values():
            expert.upload_weights()
    ...
    def forward(self, hidden_states):
        ...
        for idx in idx_mask:
            next_states[router_mask[:, :, idx]] = getattr(self.experts, "expert_{}".format(idx))(
                hidden_states=hidden_states[router_mask[:, :, idx]]
            )
...
class SwitchTransformersLayerFF(nn.Module):
    ...
    def __init__(self, krnl, context, queue, config: SwitchTransformersConfig, is_sparse=False):
        super().__init__()
        ...
        if not self.is_sparse:
            ...
            self.mlp = SwitchTransformersSparseMLP(config=config, krnl=krnl, context=context, queue=queue)
...
class SwitchTransformersBlock(nn.Module):
    def __init__(self, config, krnl, context, queue, has_relative_attention_bias=False, is_sparse=False):
        super().__init__()
        self.is_decoder = config.is_decoder
        self.is_sparse = is_sparse
        ...
        self.layer.append(SwitchTransformersLayerFF(config=config, is_sparse=self.is_sparse, krnl=krnl, context=context, queue=queue))

    def upload_weights(self):
        self.layer[-1].upload_weights()
...
class SwitchTransformersStack(SwitchTransformersPreTrainedModel):
    def __init__(self, config, krnl, context, queue, embed_tokens=None):
        ...
        for i in range(config.num_layers):
            ...
            self.block.append(
                SwitchTransformersBlock(config, has_relative_attention_bias=bool(i == 0), is_sparse=is_sparse, krnl=krnl, context=context, queue=queue)
            )
...
class SwitchTransformersForConditionalGeneration(SwitchTransformersPreTrainedModel):
    ...
    def __init__(self, config: SwitchTransformersConfig):
        ...
        #--- 1. Initialize FPGA ---------------
        queue = matmul_lib.generateQueue()
        context = matmul_lib.generateContext()
        krnl = matmul_lib.generateKernel()
        matmul_lib.programDevice(queue, context, krnl)
        #--------------------------------------
        ...
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = SwitchTransformersStack(config=encoder_config, krnl=krnl, context=context, queue=queue, embed_tokens=self.shared)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = SwitchTransformersStack(config=decoder_config, krnl=krnl, context=context, queue=queue, embed_tokens=self.shared)
    ...
    def upload_weights(self):
        #--- 2. Upload Experts to FPGA --------
        self.encoder.upload_weights()
        self.decoder.upload_weights()
        #--------------------------------------
